...
export default function RootLayout({ children }) {
  ...
  const showDebug = process.env.NODE_ENV !== 'production' || oidcEnabled;
  return (
    <html lang="en">
      <body className="min-h-screen bg-[#f8f9fb] text-[#1a3a32] flex flex-col">
        {showDebug && (
          <div style={{ background: '#ffeeba', color: '#856404', padding: '8px', borderRadius: '6px', margin: '16px', fontSize: '14px', fontWeight: 'bold', zIndex: 1000 }}>
            [OIDC Debug] OIDC_ENABLED: {String(process.env.OIDC_ENABLED)} | NEXT_PUBLIC_OIDC_ENABLED: {String(process.env.NEXT_PUBLIC_OIDC_ENABLED)} | OIDC active: {String(oidcEnabled)}
          </div>
        )}
